...
-- Run this in Supabase SQL editor: https://app.supabase.com/project/{project_id}/sql/new
...
CREATE TABLE IF NOT EXISTS children (
    ...
    independence_level INTEGER DEFAULT 2 CHECK (independence_level >= 1 AND independence_level <= 4),
...
-- Create sessions table for topic planning and scheduling
...
CREATE INDEX idx_sessions_scheduled_day ON sessions(scheduled_day_of_week);
...
-- RLS Policies for sessions table (access via children ownership)
...
INSERT INTO subjects (user_id, name, color) VALUES
    (auth.uid(), 'Mathematics', '#ef4444'),
    (auth.uid(), 'Language Arts', '#3b82f6'),
    (auth.uid(), 'Science', '#10b981'),
    (auth.uid(), 'History', '#f59e0b'),
    (auth.uid(), 'Art', '#8b5cf6');
*/