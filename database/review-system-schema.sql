...
-- This script extends the existing homeschool schema with review system functionality
-- Run this in Supabase SQL editor AFTER running homeschool-schema.sql
...
-- Add evidence capture fields to sessions table
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS evidence_notes TEXT;
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS evidence_photos TEXT[]; -- Array of file paths/URLs
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS evidence_voice_memo TEXT; -- File path/URL for voice recording
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS evidence_attachments TEXT[]; -- Array of additional file paths

-- Add commitment_type and skipped_from_date if they don't exist (from previous milestones)
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS commitment_type VARCHAR(20) DEFAULT 'preferred' 
    CHECK (commitment_type IN ('fixed', 'preferred', 'flexible'));
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS skipped_from_date DATE;
...
-- Insert default review slots (2×5min daily micro-sessions)
-- These will be created when children are added via the application
-- Uncomment the following to create sample review slots for existing children:

/*
-- Morning micro-review (5min before regular learning)
INSERT INTO review_slots (child_id, day_of_week, start_time, end_time, slot_type, is_active)
SELECT 
    c.id,
    d.day_num,
    '08:00:00'::time,
    ...
    true
FROM children c
CROSS JOIN (VALUES (1), (2), (3), (4), (5), (6), (7)) AS d(day_num)
WHERE c.user_id = auth.uid();

-- Evening micro-review (5min before bedtime routine)  
INSERT INTO review_slots (child_id, day_of_week, start_time, end_time, slot_type, is_active)
SELECT 
    c.id,
    d.day_num,
