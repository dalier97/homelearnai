...
<div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: true }" x-show="show" x-transition data-testid="subject-edit-modal">
    ...
    <div class="flex min-h-screen items-center justify-center p-4 relative z-50">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full relative z-50" @click.stop data-testid="modal-content">
            ...
            <div class="px-6 py-4">
                <form 
                    hx-put="{{ route('subjects.update', $subject->id) }}"
                    hx-target="#subjects-list"
                    hx-swap="innerHTML"
                    @submit="show = false; setTimeout(() => document.getElementById('subject-modal').innerHTML = '', 300)"
                    class="space-y-4"
                >
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($colors as $value => $label)
                                <option value="{{ $value }}" {{ $subject->color == $value ? 'selected' : '' }} style="color: {{ $value }}">
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
